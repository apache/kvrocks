...
func TestStreamOffset(t *testing.T) {
	...
	t.Run("XINFO GROUPS and XINFO CONSUMERS", func(t *testing.T) {
		...
		consumer2 := "c2"
		consumer3 := "c3"
		require.NoError(t, rdb.Del(ctx, stream_name).Err())
		require.NoError(t, rdb.XAdd(ctx, &redis.XAddArgs{
			Stream: stream_name,
			ID:     "1-0",
			Values: []string{"data", "a"},
		}).Err())
		require.NoError(t, rdb.XGroupCreate(ctx, stream_name, group1, "$").Err())
		r := rdb.XInfoGroups(ctx, stream_name).Val()
		...
		require.NoError(t, rdb.XGroupCreate(ctx, stream_name, group2, "$").Err())
		r = rdb.XInfoGroups(ctx, stream_name).Val()
		require.Equal(t, group2, r[1].Name)
		require.Equal(t, "2-0", r[1].LastDeliveredID)

		require.NoError(t, rdb.XGroupCreateConsumer(ctx, stream_name, group1, consumer1).Err())
		require.NoError(t, rdb.XGroupCreateConsumer(ctx, stream_name, group1, consumer2).Err())
		require.NoError(t, rdb.XGroupCreateConsumer(ctx, stream_name, group2, consumer3).Err())
		r = rdb.XInfoGroups(ctx, stream_name).Val()
		require.Equal(t, int64(2), r[0].Consumers)
		require.Equal(t, int64(1), r[1].Consumers)

		r1 := rdb.XInfoConsumers(ctx, stream_name, group1).Val()
		require.Equal(t, consumer1, r1[0].Name)
		require.Equal(t, consumer2, r1[1].Name)
		r1 = rdb.XInfoConsumers(ctx, stream_name, group2).Val()
