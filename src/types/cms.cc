...
void CMSketch::CMSDimFromProb(double error, double delta, size_t &width, size_t &depth) {
...
size_t CMSketch::IncrBy(const char* item, size_t item_len, size_t value) {
    ...
    for (size_t i = 0; i < depth_; ++i) {
        // check over the static casting the parameter to an int
...
int CMSketch::Merge(CMSketch* dest, size_t quantity, const std::vector<const CMSketch*>& src, const std::vector<long long>& weights) {
...
int CMSketch::checkOverflow(CMSketch* dest, size_t quantity, const std::vector<const CMSketch*>& src, const std::vector<long long>& weights) {
    int64_t item_count = 0;
    int64_t cms_count = 0;
