...
func basicTests(t *testing.T, rdb *redis.Client, ctx context.Context, encoding string) {
	...
	t.Run(fmt.Sprintf("ZMPOP basics - %s", encoding), func(t *testing.T) {
		...
		require.Equal(t, int64(3), rdb.ZAdd(ctx, "zseta", redis.Z{Score: 10, Member: "a"}, redis.Z{Score: 20, Member: "b"}, redis.Z{Score: 30, Member: "c"}).Val())
		...
		require.Equal(t, int64(3), rdb.ZAdd(ctx, "zsetb", redis.Z{Score: 40, Member: "d"}, redis.Z{Score: 50, Member: "e"}, redis.Z{Score: 60, Member: "f"}).Val())
		...
		require.Equal(t, int64(0), rdb.Exists(ctx, "zseta", "zsetb").Val())
	})
