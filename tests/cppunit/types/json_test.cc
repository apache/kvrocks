...
TEST_F(RedisJsonTest, ArrInsert) {
  std::vector<size_t> res;

  ASSERT_FALSE(json_->ArrInsert(key_, "$", 0, {"1"}, &res).ok());

  ASSERT_TRUE(json_->Set(key_, "$", R"({"x":1,"y":[]})").ok());
  ASSERT_TRUE(json_->ArrInsert(key_, "$.x", 0, {"1"}, &res).ok());
  ASSERT_EQ(res.size(), 1);
  ASSERT_EQ(res[0], 0);
  ASSERT_TRUE(json_->Get(key_, {}, &json_val_).ok());
  ASSERT_EQ(json_val_.Dump().GetValue(), R"({"x":1,"y":[]})");
  ...
  ASSERT_TRUE(json_->Set(key_, "$", R"({"x":[3,2,{"z":3}],"y":[]})").ok());
  ASSERT_TRUE(json_->ArrInsert(key_, "$.x", 1, {"1"}, &res).ok());
  ASSERT_EQ(res.size(), 1);
  ASSERT_EQ(res[0], 4);
  ASSERT_TRUE(json_->Get(key_, {}, &json_val_).ok());
  ASSERT_EQ(json_val_.Dump().GetValue(), R"({"x":[3,1,2,{"z":3}],"y":[]})");
  res.clear();
  ...
  ASSERT_EQ(res[0], 4);
  ASSERT_TRUE(json_->Get(key_, {}, &json_val_).ok());
  ASSERT_EQ(json_val_.Dump().GetValue(), R"({"x":[3,2,1,{"z":3}],"y":[]})");
  res.clear();

  ASSERT_TRUE(json_->Set(key_, "$", R"({"x":[3,2,{"z":3}],"y":[]})").ok());
  ASSERT_TRUE(json_->ArrInsert(key_, "$.x", -3, {"1"}, &res).ok());
  ASSERT_EQ(res.size(), 1);
  ASSERT_EQ(res[0], 4);
  ASSERT_TRUE(json_->Get(key_, {}, &json_val_).ok());
  ASSERT_EQ(json_val_.Dump().GetValue(), R"({"x":[1,3,2,{"z":3}],"y":[]})");
  res.clear();
  ...
  ASSERT_EQ(res[0], 4);
  ASSERT_TRUE(json_->Get(key_, {}, &json_val_).ok());
  ASSERT_EQ(json_val_.Dump().GetValue(), R"({"x":[3,2,{"z":3},1],"y":[]})");
  res.clear();

  ASSERT_TRUE(json_->Set(key_, "$", R"({"x":[3,2,{"z":3}],"y":[5,6,7]})").ok());
  ASSERT_TRUE(json_->ArrInsert(key_, "$..y", 0, {"1", "2", "3"}, &res).ok());
  ASSERT_EQ(res.size(), 1);
  ASSERT_EQ(res[0], 6);
  ASSERT_TRUE(json_->Get(key_, {}, &json_val_).ok());
  ASSERT_EQ(json_val_.Dump().GetValue(), R"({"x":[3,2,{"z":3}],"y":[1,2,3,5,6,7]})");
  res.clear();

  ASSERT_TRUE(json_->Set(key_, "$", R"({"x":[3,2,{"z":3}],"y":[5,6,7]})").ok());
  ASSERT_TRUE(json_->ArrInsert(key_, "$..y", 2, {"1", "2", "3"}, &res).ok());
  ASSERT_EQ(res.size(), 1);
  ASSERT_EQ(res[0], 6);
  ASSERT_TRUE(json_->Get(key_, {}, &json_val_).ok());
  ASSERT_EQ(json_val_.Dump().GetValue(), R"({"x":[3,2,{"z":3}],"y":[5,6,1,2,3,7]})");
  res.clear();

  ASSERT_TRUE(json_->Set(key_, "$", R"({"x":[3,2,{"z":3}],"y":[5,6,7]})").ok());
  ASSERT_TRUE(json_->ArrInsert(key_, "$..y", -1, {"1", "2", "3"}, &res).ok());
  ASSERT_EQ(res.size(), 1);
  ASSERT_EQ(res[0], 6);
  ASSERT_TRUE(json_->Get(key_, {}, &json_val_).ok());
  ASSERT_EQ(json_val_.Dump().GetValue(), R"({"x":[3,2,{"z":3}],"y":[5,6,1,2,3,7]})");
  ...
  ASSERT_TRUE(
      json_->Set(key_, "$", R"({"x":[1,2,{"x":[1,2,{"y":[],"z":3}],"y":[{"y":1},1,2,3]},1],"y":[4,5,6]})").ok());
  ASSERT_TRUE(json_->ArrInsert(key_, "$..y", 0, {"1", "2", "3"}, &res).ok());
  ASSERT_EQ(res.size(), 4);
  std::sort(res.begin(), res.end());
  ASSERT_EQ(res[0], 0);
  ...
  ASSERT_TRUE(json_->Get(key_, {}, &json_val_).ok());
  ASSERT_EQ(json_val_.Dump().GetValue(),
            R"({"x":[1,2,{"x":[1,2,{"y":[1,2,3],"z":3}],"y":[1,2,3,{"y":1},1,2,3]},1],"y":[1,2,3,4,5,6]})");
  res.clear();
}
