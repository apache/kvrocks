...
std::string EncodeStreamGroupMetadataKeyForTest(const std::string &ns, const std::string &user_key, uint64_t version,
                                                const std::string &group_name) {
  std::string sub_key;
  PutFixed64(&sub_key, UINT64_MAX);
  PutFixed8(&sub_key, static_cast<uint8_t>(redis::StreamSubkeyType::StreamConsumerGroupMetadata));
  PutFixed64(&sub_key, group_name.size());
  sub_key += group_name;
  return InternalKey(ComposeNamespaceKey(ns, user_key, false), sub_key, version, false).Encode();
}

std::string EncodeStreamGroupMetadataValueForTest(const redis::StreamConsumerGroupMetadata &metadata) {
  std::string value;
  PutFixed64(&value, metadata.consumer_number);
  PutFixed64(&value, metadata.pending_number);
  PutFixed64(&value, metadata.last_delivered_id.ms);
  PutFixed64(&value, metadata.last_delivered_id.seq);
  PutFixed64(&value, static_cast<uint64_t>(metadata.entries_read));
  PutFixed64(&value, metadata.lag);
  return value;
}
...
TEST_F(RedisStreamTest, XDelExAckedDoesNotTrustZeroPendingMetadata) {
  std::string stream_name = "test_xdelex_acked_zero_pending";
  ...
  add_options.next_id_strategy = *ParseNextStreamEntryIDStrategy("1-0");
  redis::StreamEntryID id;
  auto s = stream_->Add(*ctx_, stream_name, add_options, {"k", "v"}, &id);
  EXPECT_TRUE(s.ok());
  ...
  auto ns_key = ComposeNamespaceKey("stream_ns", stream_name, false);
  StreamMetadata metadata(false);
  s = stream_->GetMetadata(*ctx_, ns_key, &metadata);
  ASSERT_TRUE(s.ok());

  redis::StreamConsumerGroupMetadata group_metadata;
  group_metadata.consumer_number = 1;
  group_metadata.pending_number = 0;
  group_metadata.last_delivered_id = id;
  group_metadata.entries_read = 1;
  group_metadata.lag = 0;

  auto group_key = EncodeStreamGroupMetadataKeyForTest("stream_ns", stream_name, metadata.version, group_name);
  s = storage_->GetDB()->Put(rocksdb::WriteOptions(), storage_->GetCFHandle(ColumnFamilyID::Stream), group_key,
                             EncodeStreamGroupMetadataValueForTest(group_metadata));
  ASSERT_TRUE(s.ok()) << s.ToString();

  std::vector<int> results;
  s = stream_->DeleteEntriesWithOption(*ctx_, stream_name, {id}, redis::StreamDeleteOption::Acked, &results);
  EXPECT_TRUE(s.ok()) << s.ToString();
  ASSERT_EQ(results.size(), 1);
  EXPECT_EQ(static_cast<int>(redis::StreamEntryDeleteResult::kEntrySkipped), results[0]);

  uint64_t size = 0;
  redis::StreamLenOptions len_options;
  s = stream_->Len(*ctx_, stream_name, len_options, &size);
  EXPECT_TRUE(s.ok());
  EXPECT_EQ(1, size);
...
TEST_F(RedisStreamTest, XDelExDelRefDoesNotTrustZeroPendingMetadata) {
  std::string stream_name = "test_xdelex_delref_zero_pending";
  std::string group_name = "group1";
  std::string consumer_name = "c1";
  ...
  std::vector<int> results;
  s = stream_->DeleteEntriesWithOption(*ctx_, stream_name, {id}, redis::StreamDeleteOption::DelRef, &results);
  EXPECT_TRUE(s.ok()) << s.ToString();
  ASSERT_EQ(results.size(), 1);
  EXPECT_EQ(static_cast<int>(redis::StreamEntryDeleteResult::kEntryDeleted), results[0]);

  redis::StreamPendingOptions pending_options;
  pending_options.stream_name = stream_name;
  ...
  EXPECT_TRUE(s.ok());
  EXPECT_EQ(0, pending_infos.pending_number);
