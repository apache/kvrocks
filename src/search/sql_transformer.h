...
using TreeSelector = parse_tree::selector<
    ...
    parse_tree::remove_content::on<HasTagExpr, NumericCompareExpr, NotExpr, AndExpr, OrExpr, Wildcard, SelectExpr,
                                   FromExpr, WhereClause, OrderByClause, LimitClause>>;
  ...
  static auto Transform(const TreeNode& node) -> StatusOr<std::unique_ptr<Node>> {
    if (Is<Boolean>(node)) {
      ...
      if (Is<Identifier>(lhs) && Is<Number>(rhs)) {
        ...
        return Node::Create<ir::NumericCompareExpr>(ir::NumericCompareExpr::Negative(op),
      ...
      return Node::Create<ir::SearchStmt>(
          Node::As<ir::IndexRef>(GET_OR_RET(Transform(node->children[1]))),
          Node::As<ir::QueryExpr>(GET_OR_RET(Transform(node->children[2]))), std::make_unique<ir::Limit>(0, 0),
          std::make_unique<ir::SortBy>(ir::SortBy::Order::ASC, std::make_unique<ir::FieldRef>("x")),
