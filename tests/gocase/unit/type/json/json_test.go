...
func TestJson(t *testing.T) {
	...
	t.Run("JSON.ARRINSERT basics", func(t *testing.T) {
		...
		// index not a integer
		require.Error(t, rdb.Do(ctx, "JSON.ARRINSERT", "a", "$", "no", 1).Err())
		require.Error(t, rdb.Do(ctx, "JSON.ARRINSERT", "a", "$", 1.1, 1).Err())
		// stop not a integer
		require.Error(t, rdb.Do(ctx, "JSON.ARRINSERT", "a", "$", 1, 1.1).Err())
		// args size < 4
		require.Error(t, rdb.Do(ctx, "JSON.ARRINSERT", "a", "$", 0).Err())
	})
