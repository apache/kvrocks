...
rocksdb::Status List::push(const Slice &user_key,
  ...
  *ret = 0;
  ...
  for (const auto &elem : elems) {
    ...
    InternalKey(ns_key, index_buf, metadata.version, storage_->IsClusterEnabled()).Encode(&sub_key);
...
rocksdb::Status List::Pop(const Slice &user_key, std::string *elem, bool left) {
  ...
  std::string sub_key;
  InternalKey(ns_key, buf, metadata.version, storage_->IsClusterEnabled()).Encode(&sub_key);
...
rocksdb::Status List::Rem(const Slice &user_key, int count, const Slice &elem, int *ret) {
  *ret = 0;
  ...
  PutFixed64(&buf, index);
  InternalKey(ns_key, buf, metadata.version, storage_->IsClusterEnabled()).Encode(&start_key);
  ...
  if (to_delete_indexes.size() == metadata.size) {
    ...
    for (iter->Seek(start_key);
      ...
      if (iter->value() != elem) {
        buf.clear();
        PutFixed64(&buf, reversed ? max_to_delete_index-- : min_to_delete_index++);
        InternalKey(ns_key, buf, metadata.version, storage_->IsClusterEnabled()).Encode(&to_update_key);
    ...
    for (uint64_t idx = 0; idx < to_delete_indexes.size(); ++idx) {
      ...
      InternalKey(ns_key, buf, metadata.version, storage_->IsClusterEnabled()).Encode(&to_delete_key);
