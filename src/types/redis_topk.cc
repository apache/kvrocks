...
rocksdb::Status TopK::Query(engine::Context &ctx, const Slice& user_key, 
                            const Slice &items, bool *exists) { 
    std::string ns_key = AppendNamespacePrefix(user_key);
...
rocksdb::Status TopK::List(engine::Context &ctx, const Slice& user_key, 
