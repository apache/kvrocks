...
// subcommand registration with root command registration
TEST(SubcommandResolution, RejectsSubcommandWithoutRegisteredParent) {
  EXPECT_DEATH(RegisterToCommandTable(CommandCategory::Server, {MakeSubCmdAttr<TestCommandSub>(
                                                                   "missing-parent", "load", -2, "read-only", NO_KEY)}),
               "");
}
