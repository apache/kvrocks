...
rocksdb::Status Geo::Radius(const Slice &user_key,
  ...
  if (!store_key.empty()) {
    long result_length = geo_points->size();
    long returned_items_count = (count == 0 || result_length < count) ? result_length : count;
...
std::string Geo::EncodeGeoHash(double longitude, double latitude) {
  std::string geoalphabet = "0123456789bcdefghjkmnpqrstuvwxyz";
