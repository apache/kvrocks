...
TEST_F(RedisStreamTest, ParsingNextStreamEntryID) {
  EXPECT_ANY_THROW(ParseNextStreamEntryIDStrategy("XYX-ABC"));
  EXPECT_ANY_THROW(ParseNextStreamEntryIDStrategy("ABC-100"));
  EXPECT_ANY_THROW(ParseNextStreamEntryIDStrategy("100-ABC"));
  EXPECT_ANY_THROW(ParseNextStreamEntryIDStrategy("ABCDE"));
  EXPECT_ANY_THROW(ParseNextStreamEntryIDStrategy("-123456789"));
  EXPECT_ANY_THROW(ParseNextStreamEntryIDStrategy("123456789-"));
  EXPECT_ANY_THROW(ParseNextStreamEntryIDStrategy("*-XYZ"));
  EXPECT_ANY_THROW(ParseNextStreamEntryIDStrategy("ABC-*"));
  EXPECT_ANY_THROW(ParseNextStreamEntryIDStrategy("*-*"));
};
...
TEST_F(RedisStreamTest, NextIDFullySpecified) {
  ...
  strategy = ParseNextStreamEntryIDStrategy("1-1");
  s = strategy->GenerateID(last_id, &next_id);
  EXPECT_FALSE(s.IsOK());

  strategy = ParseNextStreamEntryIDStrategy("2-1");
  s = strategy->GenerateID(last_id, &next_id);
  EXPECT_FALSE(s.IsOK());

  strategy = ParseNextStreamEntryIDStrategy("2-2");
  s = strategy->GenerateID(last_id, &next_id);
  EXPECT_TRUE(s.IsOK());
  EXPECT_EQ(next_id.ToString(), "2-2");

  strategy = ParseNextStreamEntryIDStrategy("2-3");
  s = strategy->GenerateID(last_id, &next_id);
  EXPECT_TRUE(s.IsOK());
  EXPECT_EQ(next_id.ToString(), "2-3");

  strategy = ParseNextStreamEntryIDStrategy("3-0");
  s = strategy->GenerateID(last_id, &next_id);
  EXPECT_TRUE(s.IsOK());
  EXPECT_EQ(next_id.ToString(), "3-0");

  last_id = redis::StreamEntryID(UINT64_MAX, UINT64_MAX);
  strategy = ParseNextStreamEntryIDStrategy("11111111-22222");
  s = strategy->GenerateID(last_id, &next_id);
  EXPECT_FALSE(s.IsOK());
}
...
TEST_F(RedisStreamTest, NextIDAutoGenerated) {
  ...
  last_id = redis::StreamEntryID(UINT64_MAX, UINT64_MAX);
  strategy = ParseNextStreamEntryIDStrategy("*");
  s = strategy->GenerateID(last_id, &next_id);
  EXPECT_FALSE(s.IsOK());
}
...
TEST_F(RedisStreamTest, NextIDSpecificTimestampAnySeqNumber) {
  ...
  strategy = ParseNextStreamEntryIDStrategy("1-*");
  s = strategy->GenerateID(last_id, &next_id);
  EXPECT_TRUE(s.IsOK());
  ...
  strategy = ParseNextStreamEntryIDStrategy("100-*");
  s = strategy->GenerateID(last_id, &next_id);
  EXPECT_TRUE(s.IsOK());
...
TEST_F(RedisStreamTest, NextIDCurrentTimestampWithSpecificSeqNumber) {
  ...
  last_id = redis::StreamEntryID(100, 100);
  strategy = ParseNextStreamEntryIDStrategy("*-50");
  s = strategy->GenerateID(last_id, &next_id);
  EXPECT_TRUE(s.IsOK());
  EXPECT_GE(next_id.ms, ts);
  ...
  strategy = ParseNextStreamEntryIDStrategy("*-123456");
  s = strategy->GenerateID(last_id, &next_id);
  EXPECT_FALSE(s.IsOK());

  last_id = redis::StreamEntryID(100, UINT64_MAX);
  strategy = ParseNextStreamEntryIDStrategy("*-123456789");
  s = strategy->GenerateID(last_id, &next_id);
  EXPECT_TRUE(s.IsOK());
  EXPECT_GE(next_id.ms, ts);
