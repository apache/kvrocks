...
TEST_P(RedisBitmapTest, BitPosClearBit_BIT) {
  ...
  for (int i = 0; i < 1024 + 16; i++) {
    ...
    /// String will set a empty string value when initializing, so, when first
    /// querying, it should return -1.
    bitmap_->BitPos(key_, false, 0, -1, /*stop_given=*/false, &pos, /*bit_index=*/true);
    if (i == 0 && !use_bitmap) {
      EXPECT_EQ(pos, -1);
    } else {
      EXPECT_EQ(pos, i);
...
TEST_P(RedisBitmapTest, BitPosSetBit_BIT) {
  ...
  for (size_t i = 0; i < sizeof(start_indexes) / sizeof(start_indexes[0]); i++) {
    bitmap_->BitPos(key_, true, start_indexes[i], -1, true, &pos, /*bit_index=*/true);
    EXPECT_EQ(pos, offsets[i]);
  }
  auto s = bitmap_->Del(key_);
}
...
TEST_P(RedisBitmapTest, BitPosNegative_BIT) {
  ...
  int64_t pos = 0;
  // First bit is negative
  bitmap_->BitPos(key_, false, 0, -1, true, &pos, /*bit_index=*/true);
  EXPECT_EQ(0, pos);
  // 8 * 1024 - 1 bit is positive
  bitmap_->BitPos(key_, true, 0, -1, true, &pos, /*bit_index=*/true);
  EXPECT_EQ(8 * 1024 - 1, pos);
  ...
  bitmap_->BitPos(key_, false, -1, -1, true, &pos, /*bit_index=*/true);
  EXPECT_EQ(-1, pos);
  // Last Bit in 1023 byte is positive
  bitmap_->BitPos(key_, true, -1, -1, true, &pos, /*bit_index=*/true);
  EXPECT_EQ(8 * 1024 - 1, pos);
  // Large negative number will be normalized.
  bitmap_->BitPos(key_, false, -10000, -10000, true, &pos, /*bit_index=*/true);
  EXPECT_EQ(0, pos);
