...
struct NumericFieldScanExecutor : ExecutorNode {
  ...
  InternalKey IndexKey(double num, uint64_t version) {
    return InternalKey(ns_key, redis::ConstructNumericFieldSubkey(scan->field->name, num, {}), version,
  ...
  StatusOr<Result> Next() override {
    if (!iter) {
      ...
      if (scan->order == SortByClause::ASC) {
        iter->Seek(IndexKey(scan->range.l, index->metadata.version).Encode());
      } else {
        iter->SeekForPrev(IndexKey(IntervalSet::PrevNum(scan->range.r), index->metadata.version).Encode());
      }
    }
