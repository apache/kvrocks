...
bool Config::ignoreIfNotModified(const std::string &key, const std::string &value) const {
  auto iter = fields_.find(key);
...
Status Config::Rewrite(const std::map<std::string, std::string> &tokens) {
  ...
  for (const auto &remain : new_config) {
    if (remain.second.empty() || ignoreIfNotModified(remain.first, remain.second)) continue;
