...
Status EvalGenericCommand(redis::Connection *conn, const std::string &body_or_sha, const std::vector<std::string> &keys,
  ...
  if (lua_isnil(lua, -1)) {
    ...
    if (evalsha) {
      ...
      if (!s.IsOK()) {
        ...
        *output = redis::Error(redis::ErrorType::NoScript, redis::errNoMatchingScript);
        return Status::OK();
      }
    } else {
      body = body_or_sha;
  ...
  if (lua_pcall(lua, 0, 1, -2)) {
    ...
    *output = redis::Error(redis::ErrorType::Err, msg);
    lua_pop(lua, 2);
  } else {
    *output = ReplyToRedisReply(conn, lua);
    lua_pop(lua, 2);
...
std::string ReplyToRedisReply(redis::Connection *conn, lua_State *lua) {
  ...
  switch (t) {
    ...
    case LUA_TTABLE:
      ...
      t = lua_type(lua, -1);
      if (t == LUA_TSTRING) {
        output = redis::Error(redis::ErrorType::None, lua_tostring(lua, -1));
        lua_pop(lua, 1);
        return output;
      }
