...
rocksdb::Status ZSet::Add(const Slice &user_key, uint8_t flags, std::vector<MemberScore> *mscores, int *ret) {
  *ret = 0;
  ...
  for (size_t i = 0; i < mscores->size(); i++) {
    InternalKey(ns_key, (*mscores)[i].member, metadata.version, storage_->IsClusterEnabled()).Encode(&member_key);
    if (metadata.size > 0) {
      ...
      if (s.ok()) {
        ...
        if ((*mscores)[i].score != old_score) {
          ...
          InternalKey(ns_key, old_score_bytes, metadata.version, storage_->IsClusterEnabled()).Encode(&old_score_key);
          ...
          InternalKey(ns_key, new_score_bytes, metadata.version, storage_->IsClusterEnabled()).Encode(&new_score_key);
    ...
    InternalKey(ns_key, score_bytes, metadata.version, storage_->IsClusterEnabled()).Encode(&score_key);
...
rocksdb::Status ZSet::Pop(const Slice &user_key, int count, bool min, std::vector<MemberScore> *mscores) {
  ...
  InternalKey(ns_key, score_bytes, metadata.version, storage_->IsClusterEnabled()).Encode(&start_key);
  ...
  for (;
    ...
    InternalKey(ns_key, score_key, metadata.version, storage_->IsClusterEnabled()).Encode(&default_cf_key);
...
*mscores) {
  ...
  for (;
    ...
    if (count >= start) {
      if (removed) {
        std::string sub_key;
        InternalKey(ns_key, score_key, metadata.version, storage_->IsClusterEnabled()).Encode(&sub_key);
...
rocksdb::Status ZSet::RangeByScore(const Slice &user_key,
  ...
  std::string start_key, prefix_key;
  InternalKey(ns_key, start_score_bytes, metadata.version, storage_->IsClusterEnabled()).Encode(&start_key);
  InternalKey(ns_key, "", metadata.version, storage_->IsClusterEnabled()).Encode(&prefix_key);
...
rocksdb::Status ZSet::RangeByLex(const Slice &user_key,
  ...
  std::string start_key, prefix_key;
  InternalKey(ns_key, spec.min, metadata.version, storage_->IsClusterEnabled()).Encode(&start_key);
  InternalKey(ns_key, "", metadata.version, storage_->IsClusterEnabled()).Encode(&prefix_key);
...
rocksdb::Status ZSet::Score(const Slice &user_key, const Slice &member, double *score) {
  ...
  InternalKey(ns_key, member, metadata.version, storage_->IsClusterEnabled()).Encode(&member_key);
...
rocksdb::Status ZSet::Overwrite(const Slice &user_key, const std::vector<MemberScore> &mscores) {
  ...
  for (const auto &ms : mscores) {
    ...
    InternalKey(ns_key, ms.member, metadata.version, storage_->IsClusterEnabled()).Encode(&member_key);
