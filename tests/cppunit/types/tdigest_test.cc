...
TEST_F(RedisTDigestTest, RevRank_and_Rank_on_the_set_containing_different_elements) {
  ...
  status = tdigest_->Rank<true>(*ctx_, test_digest_name, value, result);
  ...
  result.clear();
  result.reserve(input.size());
  status = tdigest_->Rank<false>(*ctx_, test_digest_name, value, result);
...
TEST_F(RedisTDigestTest, RevRank_and_Rank_on_different_or_same_and_unordered_inputs_tdigest) {
  ...
  status = tdigest_->Rank<true>(*ctx_, test_digest_name, value_new, result);
